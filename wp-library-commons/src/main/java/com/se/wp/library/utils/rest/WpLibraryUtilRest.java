...
@Component
public class WpLibraryUtilRest {
	private static final Logger LOGGER = LoggerFactory.getLogger(WpLibraryUtilRest.class);
	@Autowired
	...
	public void addPageViewDataLayer(String countryCode, String languageCode, String refNum, String docType,
		...
		try {
			...
			boolean ga4Enabled = Boolean
					.parseBoolean(propertiesUtilRest.getProperty("wp-library.properties", "ga.4.enabled"));
			model.addAttribute("ga4Enabled", ga4Enabled);
			if (Boolean.parseBoolean(propertiesUtilRest.getProperty("wp-library.properties", "ga.enabled"))) {
				model.addAttribute("gaPageViewEnabled", true);
				model.addAttribute("gtmId", propertiesUtilRest.getProperty("wp-library.properties", "gtm.id.ga3"));
				if (ga4Enabled)
					model.addAttribute("gtmId", propertiesUtilRest.getProperty("wp-library.properties", "gtm.id.ga4"));
				Map<String, String> gaPageViewDataLayerProperties = propertiesUtilRest
				...
				if (null != gaPageViewDataLayerProperties && !gaPageViewDataLayerProperties.isEmpty()) {
					...
					gaPageViewDataLayer
							.setEnvironment(propertiesUtilRest.getProperty("wp-library.properties", "ga.environemt"));
	...
	private void addDownloadDataLayer(Model model) throws IOException {
		...
		Map<String, String> gaDownloadDataLayerProperties= propertiesUtilRest.getAllProperties(propertiesFileName);
	...
	public Map<String,String> getMetaSpecificationData(){
		...
		try {
			metaConfigData.putAll(propertiesUtilRest.getAllProperties(Constants.META_CONFIG_FILE_NAME));
		} catch (IOException e) {
	...
	public void addOneTrustConfig(String language, Model model) {
		...
		try {
			oneTrustId = propertiesUtilRest.getProperty("wp-library.properties", "one.trust.script.id");
		} catch (IOException e) {
			LOGGER.error("Error reading global one trust id.", e);
		}
		if ("zh".equalsIgnoreCase(language)) {
			try {
				oneTrustId = propertiesUtilRest.getProperty("document-types.properties",
